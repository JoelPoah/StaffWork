<template>
  <v-app>
        ...
        <v-row no-gutters class="fill-height">
          ...
          <v-col cols="12" md="6" class="left-panel">
            <v-sheet class="fill-height pa-4" color="#C6ECAE">
              ...
              <FileUpload @document-loaded="handleDocumentLoaded" @errors-received="handleErrors"
                @word-errors-received="handleWordErrors" @images-received="handleImages"
                @image-references-received="handleImageReferences" @file-selected="handleFileSelected" />
              ...
              <v-expand-transition>
                <div v-if="hasErrors" class="mt-4">
                  ...
                  <div v-if="errors.summary && errors.summary.length > 0">
                    <v-alert v-for="(error, idx) in errors.summary" :key="'summary-' + idx"
                      :type="error.type || 'warning'" class="mb-2" variant="tonal" density="compact" border="start">
                      {{ error.message }}
                    </v-alert>
                  </div>
                  ...
                  <div v-if="errors.formatting && errors.formatting.length > 0">
                    <v-alert type="warning" class="mb-2" variant="tonal" density="compact" border="start"
                      color="#777DA7">
                  ...
                  <div v-if="errors.fonts && errors.fonts.length > 0">
                    <v-alert type="error" class="mb-2" variant="tonal" density="compact" border="start" color="#D5573B">
                  ...
                  <div v-if="errors.images && errors.images.length > 0">
                    <v-alert v-for="(error, idx) in errors.images" :key="'image-' + idx" :type="error.type || 'warning'"
                      class="mb-2" variant="tonal" density="compact" border="start" color="#777DA7">
                      {{ error.message }}
                    </v-alert>
                  </div>

                  <!-- Instruction for users -->
                  <v-alert type="info" class="mt-2" variant="tonal" density="compact" border="start" color="#94C9A9">
                    <ul class="mb-0 ps-4">
                      <li>Hover over highlighted words to see specific formatting errors</li>
                      <li v-if="hasImageReferences">Image references are highlighted - valid references in green,
                        missing references in orange</li>
                    </ul>
                  </v-alert>
                </div>
              </v-expand-transition>
          ...
          <v-col cols="12" md="6" class="right-panel">
            <ChatInterface ref="chatInterfaceRef" :document-loaded="documentLoaded" :document-errors="errors"
              :document-content="documentContent" @message-sent="handleMessageSent" />
          </v-col>
        </v-row>
      </v-container>
    </v-main>
    ...
    <v-tooltip v-model="showTooltip" :activator="tooltipActivator" location="top" content-class="error-tooltip">
    ...
    <v-tooltip v-model="showImageRefTooltip" :activator="imageRefTooltipActivator" location="top"
